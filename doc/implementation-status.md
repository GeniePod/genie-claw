...
| Tool-call evaluation | Implemented | `crates/genie-core/src/eval/bfcl.rs`, `crates/genie-ctl/src/main.rs`, `crates/genie-ctl/src/bfcl_import.rs`, `tests/bfcl/*` | BFCL-style JSONL scoring for parsed tool names and JSON arguments, plus local Home Assistant Intents import, deterministic quick-router prediction generation, and local-LLM prediction generation. It is side-effect free and does not execute tools or require a live home backend. Current 2026-05-29 HA English quick-router baseline: 208 cases, 0 missing predictions, 5.77% strict accuracy. |
