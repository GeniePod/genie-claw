...
| Landlock filesystem sandbox (genie-core) | Implemented (Linux) | `crates/genie-core/src/security/sandbox.rs`, `landlock_rules.rs`, `[sandbox]` in config | `apply_landlock` runs at startup; `/api/health` reports `sandbox_enforced`. Per-skill worker isolation remains planned (`genie-skill-sdk/src/sandbox.rs`). |
