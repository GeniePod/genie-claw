...
- `setup-jetson.sh [5f/6]` now captures a 3-second silent recording from
  the detected capture device at install time and runs it through
  `sox noiseprof` to generate a per-host microphone noise profile at
  `/opt/geniepod/data/mic-noise.prof`. The operator is prompted to stay
  silent during the capture. If sox is missing or arecord fails, the
  step is skipped cleanly and the capture pipeline falls back to its
  compand-only behaviour. Re-run setup to refresh the profile after
  moving the Jetson or LyraT to a different room.

### Changed

- `record_audio`'s sox preprocessing chain now conditionally applies
  `noisered <profile> 0.21` for spectral noise subtraction when
  `/opt/geniepod/data/mic-noise.prof` exists (generated by
  `setup-jetson.sh [5f/6]`). 0.21 is conservative — picked so the
  subtraction is audibly helpful on the ES8388 ADC noise floor without
  producing the "musical noise" artifact that aggressive spectral
  subtraction is known for.
- compand transfer function tightened to add an aggressive gate below
  -65 dBFS (input -65 dBFS → output -90 dBFS = effective silence). New
  curve `-65,-90,-50,-50,-25,-12,-5,-5` keeps the existing quiet-speech
  lift at -25/-12 dBFS but cleans up the silent-between-words bands.
  Speech is unaffected because its dynamic range sits above -50 dBFS.
  Closes #10.
