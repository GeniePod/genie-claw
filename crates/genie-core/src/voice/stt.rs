...
pub async fn record_audio(device: &str, sample_rate: u32, duration_secs: u32) -> Result<String> {
    ...
    // Optional spectral noise subtraction. If a per-host noise profile exists
    // (generated by `setup-jetson.sh` from a 3 s silent capture and stored at
    // /opt/geniepod/data/mic-noise.prof), sox's `noisered` effect subtracts
    // the modelled noise spectrum from the captured audio before compand and
    // peak-normalize. Without a profile we skip the step — the rest of the
    // chain still runs as before, with only the tightened compand noise gate
    // helping the silent-between-words case.
    const NOISE_PROFILE: &str = "/opt/geniepod/data/mic-noise.prof";
    let have_noise_profile = tokio::fs::metadata(NOISE_PROFILE).await.is_ok();

    let mut sox_cmd = Command::new("sox");
    sox_cmd
        .arg(wav_path.as_str())
        .arg(normalized_path.as_str())
        .args(["channels", "1"])
        .args(["highpass", "100"])
        .args(["lowpass", "7000"]);
    if have_noise_profile {
        ...
        sox_cmd.args(["noisered", NOISE_PROFILE, "0.21"]);
    }
    // compand transfer with an added aggressive gate below -65 dBFS:
    //     -65 dBFS -> -90 dBFS   (gate: noise below this is silenced)
    //     -50 dBFS -> -50 dBFS   (existing floor)
    //     -25 dBFS -> -12 dBFS   (quiet-speech lift, +13 dB)
    //     -5  dBFS -> -5  dBFS   (loud speech held)
    // Helps the silent-between-words and silent-at-start cases — speech is
    // unaffected because its dynamic range sits above -50 dBFS.
    sox_cmd.args([
        "compand",
        "0.02,0.20",
        "-65,-90,-50,-50,-25,-12,-5,-5",
        "-2",
    ]);
    ...
    match sox_cmd.output().await {
                    ...
                    .unwrap_or(false) =>
        {
            ...
            tracing::info!(
                ...
                "preprocessed audio (highpass 100, lowpass 7000, noisered if profile, compand gate+lift, peak-normalize -3 dBFS)"
            );
            Ok(normalized_path)
        }
        Ok(out) => {
