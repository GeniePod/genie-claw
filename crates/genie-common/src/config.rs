...
mod tests {
    ...
    fn api_service_has_default_probe_url() {
        let config = test_config();
        assert_eq!(config.api_service().url, "http://127.0.0.1:3080/api/status");
    }

    #[test]
    fn service_http_probe_parses_host_port_and_path() {
        let (addr, path) = service_http_probe("http://192.168.1.50:8123/api/", 8123).unwrap();
    ...
    fn service_http_probe_parses_api_status_url() {
        let (addr, path) = service_http_probe("http://127.0.0.1:3080/api/status", 3080).unwrap();
